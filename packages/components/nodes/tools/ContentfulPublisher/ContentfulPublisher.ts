import { ICommonObject, IDatabaseEntity, INode, INodeData, INodeOptionsValue, INodeParams } from '../../../src/Interface'
import { convertSchemaToZod, getBaseClasses } from '../../../src/utils'
import { DynamicStructuredTool } from '../CustomTool/core'
import contentful from 'contentful'
import { z } from 'zod'
import { DataSource } from 'typeorm'
import { getCredentialData, getCredentialParam } from '../../../src/utils'

class CustomTool_Tools implements INode {
    label: string
    name: string
    version: number
    description: string
    type: string
    icon: string
    category: string
    credential: INodeParams
    baseClasses: string[]
    inputs: INodeParams[]

    constructor() {
        this.label = 'Contentful Publisher'
        this.name = 'contentfulPublisher'
        this.version = 1.0
        this.type = 'ContentfulPublisher'
        this.icon = 'contentful.png'
        this.category = 'Tools'
        this.description = `Create content in Contentful`
        this.credential = {
            label: 'Connect Credential',
            name: 'credential',
            type: 'credential',
            credentialNames: ['contentfulManagementApi']
        }
        this.inputs = [
            {
                label: 'Select Tool',
                name: 'selectedTool',
                type: 'asyncOptions',
                loadMethod: 'listTools'
            }
        ]
        this.baseClasses = [this.type, 'Tool', ...getBaseClasses(DynamicStructuredTool)]
    }
    // @ts-ignore
    loadMethods = {
        async listTools(_: INodeData, options: ICommonObject): Promise<INodeOptionsValue[]> {
            console.log('GOT HERE')
            debugger
            const credentialData = await getCredentialData(_.credential ?? '', options)
            const returnData: INodeOptionsValue[] = []

            // Get all of the content types from Contentful
            const client = contentful.createClient({
                accessToken: credentialData.accessToken,
                space: credentialData.spaceId,
                environment: _.inputs?.environmentId as string
            })
            debugger
            const contentTypes = await client.getContentTypes()

            for (let i = 0; i < contentTypes.items.length; i += 1) {
                const data = {
                    label: contentTypes.items[i].name,
                    name: contentTypes.items[i].sys.id,
                    description: contentTypes.items[i].description
                } as INodeOptionsValue
                returnData.push(data)
            }
            return returnData
        }
    }

    //@ts-ignore
    // loadMethods = {
    //     async listTools(_: INodeData, options: ICommonObject): Promise<INodeOptionsValue[]> {
    //         const returnData: INodeOptionsValue[] = []

    //         const appDataSource = options.appDataSource as DataSource
    //         const databaseEntities = options.databaseEntities as IDatabaseEntity

    //         if (appDataSource === undefined || !appDataSource) {
    //             return returnData
    //         }

    //         const tools = await appDataSource.getRepository(databaseEntities['Tool']).find()

    //         for (let i = 0; i < tools.length; i += 1) {
    //             const data = {
    //                 label: tools[i].name,
    //                 name: tools[i].id,
    //                 description: tools[i].description
    //             } as INodeOptionsValue
    //             returnData.push(data)
    //         }
    //         return returnData
    //     }
    // }

    async init(nodeData: INodeData, _: string, options: ICommonObject): Promise<any> {
        const selectedToolId = nodeData.inputs?.selectedTool as string
        const customToolFunc = nodeData.inputs?.customToolFunc as string

        const appDataSource = options.appDataSource as DataSource
        const databaseEntities = options.databaseEntities as IDatabaseEntity

        try {
            const tool = await appDataSource.getRepository(databaseEntities['Tool']).findOneBy({
                id: selectedToolId
            })

            if (!tool) throw new Error(`Tool ${selectedToolId} not found`)
            const obj = {
                name: tool.name,
                description: tool.description,
                schema: z.object(convertSchemaToZod(tool.schema)),
                code: tool.func
            }
            if (customToolFunc) obj.code = customToolFunc
            return new DynamicStructuredTool(obj)
        } catch (e) {
            throw new Error(e)
        }
    }
}

module.exports = { nodeClass: CustomTool_Tools }
